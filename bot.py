...
async def generate_signals():
    ...
    async with generate_lock:
        ...
        try:
            ...
            logging.info(f"🔄 MTF Генерация сигналов: {len(pairs)} пар | Открыто: {len(open_pairs)}")
            ...
            for pair in pairs:
                ...
                try:
                    # 1H — определение тренда
                    ohlcv_1h = await asyncio.to_thread(exchange.fetch_ohlcv, pair, '1h', limit=200)
                    df_1h = pd.DataFrame(ohlcv_1h, columns=['ts', 'open', 'high', 'low', 'close', 'vol'])
                    df_1h = df_1h[['open', 'high', 'low', 'close']].astype(float)

                    df_1h['ema50'] = df_1h['close'].ewm(span=50, adjust=False).mean()
                    df_1h['ema200'] = df_1h['close'].ewm(span=200, adjust=False).mean()

                    if len(df_1h) < 150 or df_1h.iloc[-1].isna().any():
                        continue
                    ...
                    # 15m — поиск входа
                    ohlcv_15m = await asyncio.to_thread(exchange.fetch_ohlcv, pair, '15m', limit=300)
                    ...
                    logging.info(f"🚀 MTF СИГНАЛ → {pair} | {direction} (1H bias OK)")
...
async def monitor_open_signals():
    try:
        ...
        if rows:
            logging.info(f"🔍 Мониторинг: {len(rows)} открытых сигналов")
...
Вход: <b>{entry:,.4f} USDT</b>"""
                    ...
                    await broadcast_message(text)
                    logging.info(f"✅ Сигнал закрыт → #{hashtag} | {status}")
...
async def start_cmd(message: types.Message):
    ...
    await message.answer(
        ...
        "🤖 Бот использует <b>MTF стратегию</b> (1h тренд + 15m вход)\n"
        "Bollinger Bands 25 + EMA 9/21\n\n"
...
async def admin_stats(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        return
    async with aiosqlite.connect(db_path) as db:
        total = (await db.execute_fetchall("SELECT COUNT(*) FROM subscribers"))[0][0]
        open_s = (await db.execute_fetchall("SELECT COUNT(*) FROM signals WHERE status = 'open'"))[0][0]
    await message.answer(f"📊 <b>Статистика:</b>\n\nПодписчиков: <b>{total}</b>\nОткрытых сигналов: <b>{open_s}</b>")
...
async def main():
    ...
    scheduler.add_job(generate_signals, 'interval', seconds=35, replace_existing=True)
    scheduler.add_job(monitor_open_signals, 'interval', seconds=15, replace_existing=True)

    scheduler.start()
    print("🚀 Бот запущен | Стратегия: MTF (1h тренд + 15m вход) | R:R 1:2.2")
